...
MATCH_RULES = """Ты решаешь, один и тот же ли товар продаётся на двух карточках с разных
...
"""
"""Определение матча. Общее у модели подтверждения и судьи-B
(`evals/judges.py`): правка меняет промпт обоих."""
...
"""Порядок полей менять нельзя: с вердиктом в первой строке модель ставит
его до рассуждения и на чужих карточках опровергает себя (эксперимент 12 в
`evals/results.md`)."""
...
def find_candidates(
    ...
    """Похожие карточки Озона по тексту карточки ВБ. Тот же `search_products`, что у
    тула A: площадка другая, запрос — карточка целиком.
    """
