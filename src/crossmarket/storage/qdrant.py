...
Коллекции раздельные, а не одна с фильтром: tool A ищет по ВБ, tool B — кандидатов
на Озоне, и смешанная коллекция в обоих случаях требовала бы фильтра.

Идентификатор точки — UUID из пары (площадка, id): Qdrant не берёт произвольные
строки, а повторная заливка товара должна перезаписывать точку, а не плодить дубли.

В payload — идентификатор для похода в ClickHouse и два поля для фильтрации выдачи,
категория и цена. Цена дублирует ClickHouse осознанно: ценовое ограничение в tool A
применяется фильтром по payload, а не походом в tool C.
"""
...
def payload_of(product: Product, synthetic: bool = False) -> dict[str, Any]:
    """Поля точки. `synthetic` помечает дистрактор: по нему фон исключается из выдачи
    и считается, сколько документов корпуса настоящие."""
    return {
