...
def product_text(product: Product, composition: str = DEFAULT_COMPOSITION) -> str:
    ...
    Состав подбирается замером — `python -m evals retrieval --text` гоняет варианты.
    Название есть всегда, остальные части подписаны, чтобы модель видела границу
    между полями; пустые выпадают.
    """
    values = {
        "title": product.title,
