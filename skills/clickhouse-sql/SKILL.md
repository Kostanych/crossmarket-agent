...
по-разному: страна — это `Страна-изготовитель` у Ozon (217 карточек) и
`Страна производства` у Wildberries (66). `Цвет` есть у 170 карточек Ozon и только у
23 Wildberries, `Тип` и `Материал` — практически только Ozon; у Wildberries из
...
**У 15 карточек Wildberries категория пустая** (`category = ''`). В группировке по
категориям это отдельная строка, а не пропуск.

**`review_count` заполнен у 6 карточек из 427.** Это не пробел в данных: товары
...
**Описание есть не у всех:** пустое у 51 карточки Ozon и у 15 Wildberries.
