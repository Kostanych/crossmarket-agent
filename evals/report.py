...
def write_report(name: str, title: str, intro: str, config: dict[str, Any], tables: list[Table]) -> Path:
    ...
    settings = Table("", ["parameter", "value"], [[key, value] for key, value in config.items()])
    parts = [
        f"# {title}",
        "",
        f"Generated by `python -m evals {name.split('_')[0]}` on {date.today().isoformat()}. "
        "Overwritten by every run — history is in git.",
        "",
        intro.strip(),
        "",
        "## Configuration",
