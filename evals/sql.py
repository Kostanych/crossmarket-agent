...
Сравниваются строки результата, а не текст запроса: нормализуются округление чисел,
порядок строк, перестановка и лишние колонки.

Засчитывается любой исполнившийся запрос агента, а не только последний: получив
ответ, агент часто делает ещё один запрос за контекстом. Остановился ли он на верном
запросе — диагностика `matched_last`.

Запросы переисполняются харнессом по базе заново, форматированная выдача тула не
разбирается.
"""
...
ROUND = 2
"""До скольких знаков округляются числа перед сравнением. Эталон и ответ могут
разойтись в последнем разряде из-за разного порядка суммирования."""

MAX_PERMUTATIONS = 5000
"""Предохранитель перебора колонок: `SELECT *` даёт десять колонок, и при эталоне
из четырёх это 5040 сочетаний."""

ANSWER_ROWS = 5
"""Сколько первых строк эталона проверяются в тексте ответа. Требовать все значения
топ-10 в тексте — это мерить многословность, а не верность."""
...
def compare(actual: list[Row], expected: list[Row], ordered: bool) -> tuple[bool, bool]:
    ...
    Совпадением считается наличие в ответе подмножества колонок, дающего эталон:
    `SELECT *` на вопросе «топ-5 самых дешёвых» даёт те же строки в том же порядке
    плюс лишние поля и засчитывается. Точное совпадение по форме — отдельная цифра
    `exact_match`.
...
"""В строках эталона площадка стоит кодом, а в ответе — человеческим названием.
Без этого сопоставления чекер штрафовал за «Wildberries» вместо `wb`."""


def _flatten_numbers(text: str) -> str:
    """Свести написание чисел к виду эталона: «93 666» → «93666», «1456,64» → «1456.64».

    Обе замены работают только между цифрами. Замена всех запятых подряд рвала
    названия товаров («сумки, рюкзака» → «сумки. рюкзака»), и верные ответы
    засчитывались как неверные.
    """
...
def answer_mentions(text: str, expected: list[Row], ordered: bool) -> bool:
    """Назвал ли агент значения эталона в самом ответе.

    Проверка вторичная: основная метрика — совпадение строк. Эта ловит случай, когда
    SQL верный, а в текст ушло что-то другое.
    """
...
def grade(case: SqlCase, answer: Answer, client: Any) -> SqlGrade:
    ...
    Переисполнение бесплатно и снимает зависимость от того, как выдача была
    отформатирована для модели.
    """
...
def misses_table(rows: list[dict[str, Any]]) -> Table | None:
    """Где не совпало: вопрос, эталон и запрос агента рядом.

    Только в stdout и в дампе — в отчёт не идёт: эталонный SQL несёт имена категорий,
    а результат агента — цены и названия.
    """
